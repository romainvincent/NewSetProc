...
def get_A_R(Cyc) :
    ...
    for i in range(itera-4) :
        ...
        #FOR TRACE
        if trace1.InBetween(seuil1, seuil2) and trace2.InBetween(seuil1, seuil2) : #check if the two values are in the range
            #if yes, take the higher value
            trace_push = min(trace1.field, trace2.field) #I want the position of the FIRST jump
            traceok = True

        elif trace1.InBetween(seuil1, seuil2) :
            trace_push = trace1.field
            traceok = True

        elif trace2.InBetween(seuil1, seuil2) :
            trace_push = trace2.field
            traceok = True

        if(traceok): #only do the check if a trace is valid
            if retrace1.InBetween(seuil1, seuil2) and retrace2.InBetween(seuil1, seuil2) : #check if the two values are in the range
                #if yes, take the higher value
                retrace_push = min(retrace1.field, retrace2.field) #I want the position of the LAST jump
                retraceok = True
            elif retrace1.InBetween(seuil1, seuil2) :
                retrace_push = retrace1.field
                retraceok = True
            elif retrace2.InBetween(seuil1, seuil2) :
