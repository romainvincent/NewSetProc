...
def json_data(jsonobject, i, column) :
    ...
    if type(column) == str:
        try :
            column = get_pos_column(data_temp, column)
        except :
...
def get_pos_column(data, string):
    """
...
def get_columns(data, col_num, sweep_num):
    """
    ...
    for i in range(sweep_num):
        result.append(col_array(data["measures"][i]["data"], col_num , 0))
    return result
