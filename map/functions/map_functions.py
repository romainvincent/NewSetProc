...
from setproc.common.functions.data_handling import col_array
import numpy as np
...
def plot_profile(im):
    ...
    figure()
    data = im.get_array()
    plot( linspace(ym, Ym, size(col_array(data, floor(nbr), 0))) ,  col_array(data, floor(nbr), 0))
    return [linspace(ym, Ym, size(col_array(data, floor(nbr), 0))) ,  col_array(data, floor(nbr), 0), Vg]
