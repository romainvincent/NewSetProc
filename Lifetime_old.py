...
class Lifetime(CycleProcess):
    ...
    def GetHist_Gauss(self,bins = 400,sat=False):
        delta_t = self.delta_t
        if sat == True:
            fit_gauss = lambda x, a0,a1,x0,x1,sig0,sig1:(a0*exp(-np.square(x-x0)/
                         (2*np.square(sig0))) + a1*exp(-np.square(x-x1)/(2*np.square(sig1))))
        else:
        ...
        ax.set_xlim((-0.0535,0.0535)); ax3.set_xlim((-0.0535,0.0535))
        #trace
        
        hist1 = np.histogram(self.raw[nonzero((self.raw[:,0]%(2*delta_t))==0)][:,1],
                                      bins,range=[-0.0535,0.0535]) 
        #retrace
        hist2 = np.histogram(self.raw[nonzero((self.raw[:,0]%(2*delta_t))!=0)][:,1],
                                      bins,range=[-0.0535,0.0535]) 
        
        cl = ['grey','blue','green','red']
        c = [-0.04,-0.013,0.013,0.04]
        tr = self.data[nonzero((self.data[:,0]%(2*delta_t))==0)]
        rt = self.data[nonzero((self.data[:,0]%(2*delta_t))!=0)]
        occ = np.zeros(4)
        z = int(bins/4.0)
        wdt = int(bins/16)
        par = []
        for i in range(4):    
            if sat == True:
                params, cov = curve_fit(fit_gauss,hist1[1][i*z+wdt:(i+1)*z-int(1.7*wdt)],
                                    hist1[0][i*z+wdt:(i+1)*z-int(1.7*wdt)],
                                    [3000,3000,c[i],c[i]-0.003,0.001,0.001])
                par.append(params)
                fit = fit_gauss(hist1[1][i*z:(i+1)*z],params[0],params[1],
            ...
            ax2.bar(hist1[1][i*z:(i+1)*z],hist1[0][i*z:(i+1)*z],
                    width=4*0.0535/bins,color=cl[i],edgecolor='None')
            ax2.plot(hist1[1][i*z:(i+1)*z],fit,'k--')
            #ax.bar(params[1],np.sqrt(2*pi)*params[0]*params[2],align='center',
            #    alpha=0.3,width=6*params[2],color='grey',edgecolor='black',
            #    linewidth=2)  
            ax.bar(params[1],size(tr[nonzero(tr==(1.5-i))]),align='center',
                alpha=0.3,width=0.01,color='grey',edgecolor='black',
                linewidth=2)
            occ[i] += size(tr[nonzero(tr==(1.5-i))])
            #ax.bar(params[1],sum(hist1[0][i*50:(i+1)*50]),align='center',
            #    alpha=0.3,width=0.005,color='orange',edgecolor='black',
            #    linewidth=2)
        
        for i in range(4):
            if sat == True:
                params, cov = curve_fit(fit_gauss,hist2[1][i*z+int(1.7*wdt):(i+1)*z-wdt],
                                    hist2[0][i*z+int(1.7*wdt):(i+1)*z-wdt],
                                    [3000,3000,c[i],c[i]+0.003,0.001,0.001])
                par.append(params)
                fit = fit_gauss(hist2[1][i*z:(i+1)*z],params[0],params[1],
                                params[2],params[3],params[4],params[5])
            else:
                params, cov = curve_fit(fit_gauss,hist2[1][i*z:(i+1)*z],
                                    hist2[0][i*z:(i+1)*z],[1000,c[i],0.005])
                fit = fit_gauss(hist2[1][i*z:(i+1)*z],params[0],params[1],params[2])

            ax4.bar(hist2[1][i*z:(i+1)*z],hist2[0][i*z:(i+1)*z],
                    width=4*0.0535/bins,color=cl[i],edgecolor='None')
            ax4.plot(hist2[1][i*z:(i+1)*z],fit,'k--')
            #ax3.bar(params[1],np.sqrt(2*pi)*params[0]*params[2],align='center',
            #    alpha=0.3,width=6*params[2],color='grey',edgecolor='black',
            #    linewidth=2)  
            ax3.bar(params[1],size(rt[nonzero(rt==(1.5-i))]),align='center',
                alpha=0.3,width=0.01,color='grey',edgecolor='black',
                linewidth=2)
            occ[i] += size(rt[nonzero(rt==(1.5-i))])
            #ax3.bar(params[1],sum(hist2[0][i*50:(i+1)*50]),align='center',
            #    alpha=0.3,width=0.005,color='orange',edgecolor='black',
            #    linewidth=2)
        
        f1.tight_layout() 
        f1.savefig('Histogram.png',dpi=300,format='png')
        f1.savefig('Histogram.pdf',dpi=300,format='pdf')
    ...
    def Movie(self):
        ...
        for jj in range(1):
            ...
            ax1b.scatter(self.raw[:,0]-shift,self.raw[:,1],s=120,c="white",edgecolor="k")
            ax1b.scatter(self.raw[:,0]-shift,self.raw[:,1],s=25,c=(0.3,0.3,0.3),edgecolor=(0.3,0.3,0.3))
            ...
            #fig.show()
            fig.savefig(str('%05d' % jj)+'.png',dpi=150,format='png')
    ...
    def MissingLZ(self,his,lim=[0,100],legloc=3, save = False):
        ...
        for i in range(4):
            ax2.text(x,1.5-i,str(round(1000*his[i]/
            (his[i]+miss[i]))/10.)+"%",fontsize="x-large")            
