...
if __name__ == '__main__':
    ...
    for i in range(3):    
        ...
        ax.set_ylim((0,int(i/2.)/2.+1))
        #ax1.set_yticklabels(())
        f = lambda x,a,b,c,d: a*np.cos((x+c)/b*2*np.pi)+d
        params,dev = curve_fit(f,width,His[i],[-0.1,omega_s,0,0.2]) 
        time = np.linspace(width[0],width[-1],100)
        fit = f(time,params[0],params[1],params[2],params[3])
